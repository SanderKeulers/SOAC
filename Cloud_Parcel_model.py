...
def T_dew(wv, z):
    ...
    return 243.5*np.log(P_env(z)*wv/(611.2*(0.622+wv)))/(17.67-np.log(P_env(z)*wv/(611.2*(0.622+wv))))
...
# things to choose
tmax = 838           # integration time in seconds
...
wl_parc = np.zeros_like(time)
wv_env  = np.zeros_like(time)
wl_env  = np.zeros_like(time)
z = np.zeros_like(time)         # height (m)

# initial conditions
#mu = 0                             # entrainment rate, equals dln(m')
W[0] = 1.0                            # mu[0] is dependent on W[0], if W[0] = 0 >> mu = Inf   # initial velocity of parcel
wv_parc[0] = 0.0134                 # initial water vapour mixing ratio of parcel.
wl_parc[0] = 0                      # initial liquid water mixing ratio of parcel.
wv_env[0] = 0.0134                     # water vapour mixing ratio of environment.
wl_env[0] = 0                          # liquid water mixing ratio of environment.
...
def mu(B,W):
    a = 0.01                    # For deep convection > a*C should be in order of 0.03
    C = 0.3
    return a*C*B/(W**2)        # Entrainment according to Eqs. 2 from Chikira & Sugiyama (2010) 
...
def RH(z, obs = obs):
    ...
    f = interp1d(h, RH_atm, fill_value = 'extrapolate')
    
    return f(z)

def mu3(z,wv_sat,T):
    mu0 = 1.8*10**-3
    f_scale = (wv_sat(T,z)/(wv_sat(T[2600],z[2600])))**3
    return mu0*(1.3 - RH(z)/100)*f_scale

# adjust wv and wl in case saturation already occures at the ground
dwv, wv_parc[0], wl_parc[0], wvsat = dw(T_parc[0], z[0], wv_parc[0], wl_parc[0], delay)

# Euler forward scheme
for t in range(len(time)-1):
    wl_parc[t] = wl_parc[t] + mu(B(T_parc[t],T_env(z[t])),W[t])*wl_parc[t]*abs(W[t])*dt 
    wv_parc[t] = wv_parc[t] + mu(B(T_parc[t],T_env(z[t])),W[t])*(wv_parc[t]-wv_env[t])*abs(W[t])*dt 
    W[t+1] = W[t] + dt*(g/(1+gamma)*((T_parc[t] - T_env(z[t]))/(T_env(z[t]) + 273.15) - wl_parc[t]) - (mu(B(T_parc[t],T_env(z[t])),W[t])*W[t]*abs(W[t]))/(1+gamma))
    z[t+1] = z[t] + W[t+1]*dt
    dwv, wv_parc[t+1], wl_parc[t+1], wvsat = dw(T_parc[t], z[t+1], wv_parc[t], wl_parc[t], delay)
    T_parc[t+1] = T_parc[t] - dt*(g/cpa*W[t+1]) + (Le/cpa*(dwv) + mu(B(T_parc[t],T_env(z[t+1])),W[t])*(T_parc[t]-T_env(z[t+1]))*W[t+1])
    
# for t in range(len(time)-1):
        
# # update the velocity of the air parcel 
#     wl_parc = wl_parc + mu2(W[t])*wl_env*dt   
#     W[t+1] = W[t] + 1/(1+gamma)*(g*dt*((T_parc[t] - T_env(z[t]))/(T_env(z[t]) + 273.15) - wl_parc - mu2(W[t])*W[t]))
...
#     T_parc[t+1] = T_parc[t] - g/cpa*W[t+1]*dt + Le/cpa*(dwv + mu2(W[t])*(wv_parc[t+1] - wv_env)) -mu2(W[t])*(T_parc[t] - T_env(z[t+1]))
