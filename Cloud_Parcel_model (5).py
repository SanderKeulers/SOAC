...
def wv_env(z, obs = obs):
    """ Returns the environmental temperature given a specific height """
    
    h = obs['HGHT']
    wv_atm = obs['MIXR']
            
    f = interp1d(h, wv_atm, fill_value = 'extrapolate')
...
# things to choose
tmax = 950            # integration time in seconds
...
W[0] = 0.1                        # initial velocity of parcel
...
def B(T_parc,T_env):
    return g*(T_parc-T_env)/(T_env + 273.15)
...
# update the velocity of the air parcel    
    W[t+1] = (W[t] + 1/(1+gamma)*(g*dt*((T_parc[t] - T_env(z[t]))/(T_env(z[t]) + 273.15) - wl_parc))
              - mu(W[t],B(T_parc[t],T_env(z[t])))/(1+gamma) * W[t])
...
# update the temperature of the air parcel
    T_parc[t+1] = T_parc[t] - g/cpa*W[t+1]*dt + Le/cpa*(dwv +  mu(W[t],B(T_parc[t],T_env(z[t])))*(wv_parc[t+1]) - wv_env(z[t+1])) + mu(W[t],B(T_parc[t],T_env(z[t])))*(T_parc[t] - T_env(z[t]))*W[t+1]
