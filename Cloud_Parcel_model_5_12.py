...
# observations 
filename = 'C:/Users/Daan/Documents/Climate Physics jaar 2/SOAC/Essen 20200813 1200.txt'
...
W[0] = 0                            # initial velocity of parcel
...
# update the velocity of the air parcel 
    wl_parc = wl_parc + mu*wl_env*dt   
    W[t+1] = W[t] + 1/(1+gamma)*(g*dt*((T_parc[t] - T_env(z[t]))/(T_env(z[t]) + 273.15) - wl_parc - mu*W[t]))
...
# update the temperature of the air parcel
    T_parc[t+1] = T_parc[t] - g/cpa*W[t+1]*dt + Le/cpa*(dwv + mu*(wv_parc[t+1] - wv_env)) - mu*(T_parc[t] - T_env(z[t+1]))
